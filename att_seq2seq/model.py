...
class LSTMEncoder(chainer.Chain):
    def __init__(self, vocab_size, embed_size, hidden_size):
        ...
        with self.init_scope():
            ...
            self.xe = L.Embedding(vocab_size, embed_size, mask_zero=True)
            # self.eh = L.Linear(embed_size, 4 * hidden_size)
            self.eh = L.Dense(4 * hidden_size, input_shape=(embed_size,))
            # self.hh = L.Linear(hidden_size, 4 * hidden_size)
            self.hh = L.Dense(4 * hidden_size, input_shape=(hidden_size,))
...
class AttLSTMDecoder(chainer.Chain):
    def __init__(self, vocab_size, embed_size, hidden_size):
        ...
        with self.init_scope():
            ...
            self.ye = L.Embedding(vocab_size, embed_size, mask_zero=True)
    ...
    def __call__(self, y, c, h, f, b):
        ...
        c, h = F.lstm(c, self.eh(e) + self.hh(h) + self.fh(f) + self.bh(
            b))  # 単語ベクトル、Decoderの中間ベクトル、順向きEncoderのAttention、逆向きEncoderのAttentionを使ってLSTM
...
class Attention(chainer.Chain):
    ...
    def __call__(self, fs, bs, h):
        ...
        for f, b in zip(fs, bs):
            w = F.tanh(
                self.fh(f) + self.bh(b) + self.hh(h))  # 順向きEncoderの中間ベクトル、逆向きEncoderの中間ベクトル、Decoderの中間ベクトルを使ってウェイトの計算
