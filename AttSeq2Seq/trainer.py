...
class Trainer(object):
    ...
    def fit(self, queries, responses, train_path, epoch_num=30, batch_size=40):
        train_queries = np.vstack(np.array(queries))
        train_responses = np.vstack(np.array(responses))
        ...
        for epoch in range(self.npz_num, epoch_num):
            ...
            for i in range(0, teacher_num, batch_size):
                ...
                # <eos>をデコーダーに読み込ませる
                t = np.array([0] * encode_batch_size, dtype='int32')
                # 損失の初期化
                loss = np.zeros((), dtype='float32')
                # 精度の初期化
                accuracy = np.zeros((), dtype='float32')
                # １単語ずつデコードする
                for w in dec_words:
                    y = self.model.decode(t)
                    t = np.array(w, dtype='int32')  # 正解単語をarrayに変換
            ...
            epoch_data = "epoch: {}\t".format(epoch + 1)
